...
echo Run $one...
/usr/local/bin/python3 $tosca -f $one -o $output_dir/output_CMRepo.json -c $config_tosca -s $config_sol6
echo Run $two...
/usr/local/bin/python3 $tosca -f $two -o $output_dir/output_CSD.json -c $config_tosca -s $config_sol6
